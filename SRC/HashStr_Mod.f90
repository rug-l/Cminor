...
MODULE hashtbl
  USE String_Mod, ONLY: OPERATOR(.NES.), OPERATOR(.EQS.)
  ...
  RECURSIVE SUBROUTINE get_sll(list,key,val)
    ...
    cond=.FALSE.
    IF (ALLOCATED(list%key)) THEN
      IF ( SIZE(list%key)==SIZE(keyVec) .AND. (list%key .EQS. keyVec) ) THEN
        cond = .TRUE.
      END IF
    END IF

    ! RUG: if list%key is not allocated, fortran fails to check size(list%key) etc and errors in debug mode,
    !      so it's better to check the condition seperately to avoid this
