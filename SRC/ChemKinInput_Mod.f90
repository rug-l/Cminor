...
MODULE ChemKinInput_Mod
  ...
  USE Meteo_Mod,    ONLY: R, Rerg
  USE Unirnk_Mod,   ONLY: unirnk
  USE ChemSys_Mod,  ONLY: ReactionStruct_t, Duct_T, ListGas, ReactionSystem,  &
                        & ListAtoms, PositionAtom, ListGas2, Species_T,       &
                        & ListToHashTable, HashTableToList, SortList,         &
                        & PositionSpeciesGas, PositionSpeciesAll
  !
  USE Reac_Mod,     ONLY: nspc, lowa, lowb, lowc, lowd, lowe, lowf, lowg,     &
                        & higha, highb, highc, highd, highe, highf, highg,    &
                        & SCPerm, THAtoms, RTind, RTpar, nspc2, neq2, nreac2, &
                        & nreac, nr_G_Const, y_name, nr_press, nr_simpTB,     &
                        & nr_G_temp, nr_G_Troe, nr_Gas, ns_gas, neq, rmw,     &
                        & rrho, mw, nr_g_lind
  !
  USE Control_Mod,  ONLY: ZERO, LenName, ChemUnit, ONE, kilo, eps, milli,     &
                        & dyncm2_to_Pa, mONE
  !
  USE Sparse_Mod,   ONLY: CSR_Matrix_T, New_CSR, TB_Sparse,                   &
                        & CompressIntegerArray, CompressDoubleArray
  !
  IMPLICIT NONE
  !
  !
  CHARACTER(54), PRIVATE, PARAMETER :: CKSetSpecies='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz()'
    ...
    DO 
      ...
      IF ( io_err == 0 ) THEN
        ...
        IF ( tmpPos > 0 ) THEN 
          ...
          SCperm(i) = tmpPos
        ELSE
!          WRITE(*,*) ' Species: ',TRIM(tmpChar0)//'  not in mechanism.'
        END IF
  ...
  SUBROUTINE Read_Thermodata(ThermSwitchTemp,DatThermo,UnitThermo,nSpc)
    ...
    REAL(dp)      :: ta,tb,tc,td,te,tf,tg
    REAL(dp),ALLOCATABLE :: ThermSwitchTemp(:)
    !
    INTEGER :: i, n, cnt
    INTEGER :: idx_temp
    INTEGER :: ALLOC_ERR
      ...
      SELECT CASE (rNumber)
        CASE ('1')
          ...
          ELSE
            ...
            idx_temp = INDEX(iLine,' ', BACK=.TRUE.)
            idx_temp = SCAN(iLine(:idx_temp), '0123456789', BACK=.TRUE.)
            READ(iLine(INDEX(iLine(:idx_temp),' ', BACK=.TRUE.):idx_temp), '(F13.7)') tc

            ThermSwitchTemp(i)=tc
          END IF
       CASE ('2')
  ...
  SUBROUTINE Read_Reaction(DataReac,UnitReac)
    ...
    SELECT CASE (headline) ! units are read here, but not considered later, only cal/mole works
    ...
    DO
      ...
      ELSE IF ( INDEX(iLine,'=')>0 ) THEN
        STOP 'Cminor requires input of reversible reactions as educts <=> products, not educts=products, please adjust the *.sys file.'
      END IF
      !
    END DO
    REWIND UnitReac
...
!         FUNCTIONS FOR CONVERTING MOLE FRACTION; MASS FRACTION; MOLAR CONCENTRATION 
  ...
  FUNCTION MoleFr_To_MoleConc(MoleFr,rho,Press,Temp) RESULT(MoleConc)
    REAL(dp), ALLOCATABLE :: MoleConc(:)      ! Mole concentration [mol/cm3]
    REAL(dp), INTENT(IN)  :: MoleFr(:)        ! Mole fraction [mol/mol]
...
!***************************************************************************************************
  ...
  SUBROUTINE Setup_ReactionIndex()
    ...
    ALLOCATE( RTind%iArr(nreac),  RTind%iLind(nreac), RTind%iTroe(nreac),      &
              RTind%iEqui(nreac), RTind%iXrev(nreac), RTind%iTBody(nreac),     &
              RTind%iHigh(nreac), RTind%iLow(nreac),  RTind%iTBodyExtra(nreac) )
    ...
    ALLOCATE( RTpar%A(nreac),    RTpar%b(nreac),    RTpar%E(nreac),   &
              RTpar%A0(nreac),   RTpar%b0(nreac),   RTpar%E0(nreac),  &
              RTpar%AX(nreac),   RTpar%bX(nreac),   RTpar%EX(nreac),  &
              RTpar%Ainf(nreac), RTpar%binf(nreac), RTpar%Einf(nreac),&
              RTpar%T1(nreac),   RTpar%T2(nreac),                     &
              RTpar%T3(nreac),   RTpar%T4(nreac)                      )
    ...
    DO iR=1,nreac
      ...
      IF (ReactionSystem(iR)%TB.OR.ReactionSystem(iR)%TBextra) THEN
        ...
        IF (ReactionSystem(iR)%TBextra) THEN
          ...
          cnt = cnt + SIZE(ReactionSystem(iR)%TBidx)
        END IF
      END IF
      ...
      IF (ALLOCATED(ReactionSystem(iR)%TroeConst)) THEN
        ...
        RTpar%T2(iR) = MAX( ABS(ReactionSystem(iR)%TroeConst(2)) , eps ) * SIGN( ONE , ReactionSystem(iR)%TroeConst(2) )  ! |r| >= eps
        RTpar%T3(iR) = MAX( ABS(ReactionSystem(iR)%TroeConst(3)) , eps ) * SIGN( ONE , ReactionSystem(iR)%TroeConst(3) )  ! |r| >= eps
    ...
    CALL Build_TB_sparse(TB_sparse,cnt)
    ...
    DO i = 1 , nreac
      IF (ALLOCATED(ReactionSystem(i)%TBidx)) THEN
        DO j=1,SIZE(ReactionSystem(i)%TBidx)
          ...
          ELSE
            WRITE(*,*) ' Warning :: Third body species: ',ReactionSystem(i)%TBspc(j), ' not found.'
          END IF
        END DO
      END IF
    END DO
  ...
  SUBROUTINE InsertSpecies(Species,Type)
    ...
    ! only gaseous species in combustion chemistry   
